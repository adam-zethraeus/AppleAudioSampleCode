...
`PlaylistViewController` is a table view controller subclass that manages the items
 in the playlist. It is a subview of the `ControlsViewController` that manages the
 playback controls.
*/
...
class PlaylistViewController: UITableViewController {
    ...
    // The player that contains the playlist managed by this table view.
    ...
    // The list of audio files in the app bundle.
    ...
    private func createOriginalPlaylist() {
        ...
        var newItems = PlaylistViewController.initialItems.map { PlaylistItem(title: $0.0, artist: $0.1) }
    ...
    // Note that the system may request the number of rows before the player reference is available
    ...
    // is in editing mode. You can only rearrange in that mode.
    ...
    // Note that you call these using same-named action methods in ControlsViewController.
    ...
    // Note that you call this from the ControlsViewController.
