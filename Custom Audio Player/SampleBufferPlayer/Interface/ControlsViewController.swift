...
`ControlsViewController` controls the presentation of the playback UI controls,
 such as play/pause buttons, playback times, and so forth. It has a single
 subview controller of class `PlaylistViewController` that manages the playlist.
*/
...
class ControlsViewController: UIViewController, RemoteCommandHandler {
    ...
    // The subview controller that maintains the playlist table view.
    ...
    private func updateCurrentItemInfo() {
        ...
        if let currentItem = sampleBufferPlayer.currentItem {
            ...
            // Also make sure to update the Now Playing info with
    ...
    // that button is in a hidden state, and an instance of AVRoutePickerView
    // displays instead.
